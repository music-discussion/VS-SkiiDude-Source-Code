...
class PlayState extends MusicBeatState
{
	...
	private var tric:Character;
	private var bobb:Character;
	private var whitty:Character;
	private var zard:Character;
	private var gf:Character;
	private var boyfriend:Boyfriend;
	private var invisBF:Boyfriend;

	private var eteled:Character;
	private var bobBosip:Character;
	private var skiiDude:Character;
	private var sans:Character;
	...
	private var floatshag:Float = 0;
	private var floatmatt:Float = 0;
	private var floattrick:Float = 0;
	private var floatbob:Float = 0;
	private var floatwhit:Float = 0;
	private var floatzard:Float = 0;
	private var godModeFloat:Bool = false;

	private var iconP1:HealthIcon;
	private var iconP2:HealthIcon;
	private var ghostIcon:HealthIcon;
	...
	var zardSound:FlxSound;
	var shaggerSound:FlxSound;
	...
	var shagStartpos:FlxPoint;
	var mattStartpos:FlxPoint;
	var trickStartpos:FlxPoint;
	var bobStartpos:FlxPoint;
	...
	var trickDad:Bool = false;
	var bobDad:Bool = false;
	var whitDad:Bool = false;
	var zardDad:Bool = false;
	var isEteled:Bool = false;
	var isTankman:Bool = false;
	var isSans:Bool = false;
	var isBobBosip:Bool = false;
	var isSkiiDude:Bool = false;
	var isbf:Bool = false;
	var isdad:Bool = false;
	var eyebob:FlxSprite;
	var sharty:FlxSprite;
	...
	private var shaggyTrail:FlxTrail;
	private var mattTrail:FlxTrail;
	private var trickyTrail:FlxTrail;
	private var bobTrail:FlxTrail;
	private var whitTrail:FlxTrail;
	private var zardTrail:FlxTrail;
	...
	var dSound = 0;
	var dSoundList:Array<String> = ['fnf_loss_sfx'];
	...
	*/
		...
		else if (SONG.song.toLowerCase() == 'revenge' || SONG.song.toLowerCase() == 'final-destination')
		{
			...
			tankman = new Character (-360, 40, 'tankman');
			sans = new Character (-410, 110, 'sans');
			eteled = new Character (350, 380, "eteled");
			skiiDude = new Character (350, 380, "skiiDude");
			bobBosip = new Character (350, 380, "bobBosip");
			...
			zardSound = new FlxSound().loadEmbedded(Paths.sound('fnf_loss_zardy'));
			FlxG.sound.list.add(zardSound);
			
			shaggerSound = new FlxSound().loadEmbedded(Paths.sound('fnf_loss_shaggy'));
			FlxG.sound.list.add(shaggerSound);
		...
		if (exDad)
		{
			dad2 = new Character(280, 90, "dad");
		}

		if (trickDad)
		{
			tric = new Character ( -220, 150, "tricky");
		}
		
		if (bobDad)
		{
			bobb = new Character (350, 380, "bob");
		}
		
		if (whitDad)
		{
			whitty = new Character ( -410, 110, 'whitty');
			if (Main.god)
			{
				whitty.x = -450;
				whitty.y = 150;
		...
		if (zardDad)
		{
			zard = new Character (360, 40, 'zardy');
			zard.alpha = 0;
		}
	...
	var isSkiiDude:Bool = false;*/
		...
		invisBF = new Boyfriend(1200, 450, 'bf');
		invisBF.alpha = 0;
	...
	var isSkiiDude:Bool = false;*/
		...
		if (zardDad) add(zard);
		if (exDad) add(dad2);
		if (trickDad) add(tric);
		if (bobDad) add(bobb);
		if (isTankman) add(tankman);
		if (isBobBosip) add(bobBosip);
		if (isSkiiDude) add(skiiDude);
		if (isEteled) add(eteled);
		if (isSans) add(sans);
		add(dad);
		if (whitDad) add(whitty);
		add(invisBF);
		add(boyfriend);
		add(cum);	
		...
		if (Main.god)
		{
			var cumEngineWatermark = new FlxText(4, FlxG.height - 4, 0, SONG.song + " " + (storyDifficulty == 2 ? "GOD" : storyDifficulty == 1 ? "Normal" : "Easy") + " - KE " + MainMenuState.kadeEngineVer, 16);
			cumEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
			cumEngineWatermark.scrollFactor.set();
			add(cumEngineWatermark);
		}
		else
		{
		...
		var dudewatermark = new  FlxText(4, FlxG.height - 20, 0, "Made by dude0216", 16);
		dudewatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		dudewatermark.scrollFactor.set();
		add(dudewatermark);
		...
		ghostIcon = new HealthIcon(SONG.player1, true);
		ghostIcon.y = healthBar.y - (iconP1.height / 2);
		ghostIcon.alpha = 0;
		add(iconP1);
		add(ghostIcon);
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / (1000 / c_div), function(tmr:FlxTimer)
		{
			...
			gf.dance();
			if (exDad) dad2.dance();
			if (trickDad) tric.dance();
			if (whitDad) whitty.dance();
			if (zardDad) zard.dance();
			if (bobDad) bobb.dance();
			/*
			if (isTankman) add(tankman);
		if (isBobBosip) add(bob);
		if (isSkiiDude) add(skiiDude);
		if (isEteled) add(eteled);
		if (isSans) add(sans);
		*/
			tankman.playAnim('idle');
			eteled.playAnim('idle');
			sans.playAnim('idle');
			skiiDude.playAnim('idle');
			bobBosip.playAnim('idle');
			invisBF.playAnim('idle');
		...
		switch (mania)
				{
					...
					case 2: 
						NoteSplash.colors = ['purple', 'blue', 'green', 'darkred', 'white', 'yellow', 'violet', 'red', 'darkblue'];
				}
	...
	override public function update(elapsed:Float)
	{
		...
		if (godModeFloat)
		{
			dad.y += Math.sin(floatmatt);
			dad2.y += Math.sin(floatshag);
			tric.y += Math.sin(floattrick);
			bobb.y += Math.sin(floatbob);
			whitty.y += Math.sin(floatwhit);
			zard.y += Math.sin(floatzard);
		}
		...
		if (FlxG.keys.justPressed.NINE)
		{
			...
			if (ghostIcon.animation.curAnim.name == 'bf-old')
				ghostIcon.animation.play(SONG.player1);
			else
				ghostIcon.animation.play('bf-old');
		}
		...
		ghostIcon.setGraphicSize(Std.int(FlxMath.lerp(150, ghostIcon.width, 0.50)));
		...
		var ghostIconOffset:Int = -100; 

		iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01) - iconOffset);
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (iconP2.width - iconOffset);
		ghostIcon.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01) - ghostIconOffset);
		...
		if (healthBar.percent < 20)
		{
			...
			ghostIcon.animation.curAnim.curFrame = 1;
		}
		else
		{
			iconP1.animation.curAnim.curFrame = 0;
			ghostIcon.animation.curAnim.curFrame = 0;
		}
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null && !songEnded)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				if (trickDad == true)
					camFollow.setPosition(tric.getMidpoint().x + 150, tric.getMidpoint().y - 50);
					
				if (bobDad == true)
					camFollow.setPosition(bobb.getMidpoint().x, bobb.getMidpoint().y - 100);
					
				if (whitDad == true)
					camFollow.setPosition(whitty.getMidpoint().x + 150, whitty.getMidpoint().y - 100);
					
				if (zardDad == true)
					camFollow.setPosition(zard.getMidpoint().x, zard.getMidpoint().y - 90);

				if (isTankman == true)
					camFollow.setPosition(tankman.getMidpoint().x + 150, tankman.getMidpoint().y - 100);

				if (isSans == true)
					camFollow.setPosition(sans.getMidpoint().x + 150, sans.getMidpoint().y - 100);

				if (isBobBosip == true)
					camFollow.setPosition(bobBosip.getMidpoint().x + 150, bobBosip.getMidpoint().y - 100);

				if (isEteled == true)
					camFollow.setPosition(eteled.getMidpoint().x + 150, eteled.getMidpoint().y - 100);

				if (isSkiiDude == true)
					camFollow.setPosition(skiiDude.getMidpoint().x + 150, skiiDude.getMidpoint().y - 100);
		...
		if (isSans) add(sans)
			...
			bob.playAnim('idle');*/
				...
				switch (dad.curCharacter)
				{
					...
					case 'tricky':
						camFollow.setPosition(tric.getMidpoint().x + 50, tric.getMidpoint().y);
					case 'bob':
						camFollow.setPosition(bobb.getMidpoint().x, bobb.getMidpoint().y);
					case 'pshaggy':
		...
		if (health <= 0)
		{
			...
			if (Main.god)
			{
				if (exDad == true)
				{
					...
					bobb.visible = false;
					new FlxTimer().start(0.9, function(whatthefuck:FlxTimer)
					{
						FlxG.switchState(new PlayState());
					});
				}
				...
				else if (trickDad == true)
				{
					FlxG.sound.play(Paths.sound('fnf_loss_tricky'));
					FlxG.switchState(new PlayState());
				}
				else if (bobDad == true)
				{
					FlxG.sound.play(Paths.sound('fnf_loss_bob'));
					FlxG.switchState(new PlayState());
				}
				else if (whitDad == true)
				{
					FlxG.sound.play(Paths.sound('fnf_loss_whitty'));
					FlxG.switchState(new PlayState());
				}
				else if (zardDad == true)
				{
					sharty = new FlxSprite().loadGraphic(Paths.image('zardy_fardy', 'shared'));
					sharty.scrollFactor.set(0, 0);
					sharty.antialiasing = true;
					sharty.screenCenter();
					add(sharty);
					shakeCock = true;
					camHUD.visible = false;
					zardyJumpsound();
					shaggerSound.volume = 0;
					new FlxTimer().start(0.9, function(whatthefuck:FlxTimer)
					{
						FlxG.switchState(new PlayState());
						zardSound.volume = 0;
		...
		if (generatedMusic)
			{
					...
					{	
						...
						if (!daNote.mustPress && daNote.wasGoodHit)
						{
							...
							if (SONG.notes[Math.floor(curStep / 16)] != null) //credit goes to thezoroforce240
							{
								...
								if (trickDad)
								{
									tric.playAnim('sing' + dadsDir[daNote.noteData], true);
									tric.holdTimer = 0;
								}
								if (bobDad)
								{
									bobb.playAnim('sing' + dadsDir[daNote.noteData], true);
									bobb.holdTimer = 0;
								}
								if (whitDad)
								{
								...
								if (zardDad)
								{
									zard.playAnim('sing' + dadsDir[daNote.noteData], true);
									zard.holdTimer = 0;
								}
								if (isdad)
								{
							...
							if (trickDad) tric.holdTimer = 0;
							if (bobDad) bobb.holdTimer = 0;
							if (whitDad) whitty.holdTimer = 0;
							if (zardDad) zard.holdTimer = 0;
	...
	function endSong():Void
	{
		...
		else
		{
			...
			if (Main.god) FlxG.openURL('https://cdn.discordapp.com/emojis/829960410173014016.gif?v=1');
		...
		if (!boyfriend.stunned)
		{
				...
				switch (direction)
			{
				case 0:
					...
					invisBF.playAnim('singLEFTmiss', true);
				case 1:
					boyfriend.playAnim('singDOWNmiss', true);
					invisBF.playAnim('singDOWNmiss', true);
				case 2:
					boyfriend.playAnim('singUPmiss', true);
					invisBF.playAnim('singUPmiss', true);
				case 3:
					boyfriend.playAnim('singRIGHTmiss', true);
					invisBF.playAnim('singRIGHTmiss', true);
				case 4:
					boyfriend.playAnim('singDOWNmiss', true);
					invisBF.playAnim('singDOWNmiss', true);
				case 5:
	...
	function noteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		...
		function goodNoteHit(note:Note):Void
			{
				...
				if (!note.wasGoodHit)
				{
					switch (note.noteType)
					{
						...
						if (isBobBosip)
							{
								bobBosip.playAnim('sing' + sDir[note.noteData], true);
								bobBosip.holdTimer = 0;
							}
						
						if (isSkiiDude)
							{
								skiiDude.playAnim('sing' + sDir[note.noteData], true);
								skiiDude.holdTimer = 0;
							}
						
						if (isEteled)
							{
								eteled.playAnim('sing' + sDir[note.noteData], true);
								eteled.holdTimer = 0;
							}
						
						if (isSans)
							{
								sans.playAnim('sing' + sDir[note.noteData], true);
								sans.holdTimer = 0;
							}
		...
		*/
						...
						//	invisBF.playAnim('sing' + sDir[note.noteData], true);
	...
	function zardyJumpsound()
	{
		if (!zardSound.playing)
			zardSound.play(true);
	}
	
	function shagSound()
	{
		if (!shaggerSound.playing)
			shaggerSound.play(true);
	}
	...
	*/
		...
		if (SONG.song.toLowerCase() == 'final-destination' && curStep != stepOfLast)
		{
			switch(curStep)
			{
				case 1:
					resetCharacters();
					switchCharacter('dad');
					FlxTween.tween(zard, {alpha: 1}, 2, {
            ease: FlxEase.expoOut
				});
				if (Main.god)
				{
					shaggyTrail = new FlxTrail(dad2, null, 5, 10, 0.1, 0.01);
					mattTrail = new FlxTrail(dad, null, 5, 10, 0.1, 0.01);
					trickyTrail = new FlxTrail(tric, null, 5, 10, 0.1, 0.01);
					bobTrail = new FlxTrail(bobb, null, 5, 10, 0.1, 0.01);
					whitTrail = new FlxTrail(whitty, null, 5, 10, 0.1, 0.01);
					zardTrail = new FlxTrail(zard, null, 5, 10, 0.1, 0.01);
				}
				case 128:
					resetCharacters();
					switchCharacter('sans');
				//	remove('invisBF');
				case 256:
					resetCharacters();
            ...
            ease: FlxEase.expoOut
				...
				case 1216:
					shakeCam = false;
					FlxTween.tween(invisBF, {alpha: 0.5}, 1, {
            ease: FlxEase.expoOut
				});
				FlxTween.tween(ghostIcon, {alpha: 0.5}, 1, {
            ease: FlxEase.expoOut
				});
				case 1280:
					FlxTween.tween(FlxG.camera, {zoom: 0.65}, 3.2, {
            ease: FlxEase.expoOut
				...
				shakeCam = true;
				FlxTween.tween(invisBF, {alpha: 0}, 1, {
            ease: FlxEase.expoOut
				});
				FlxTween.tween(ghostIcon, {alpha: 0}, 1, {
            ease: FlxEase.expoOut
				});
				case 1344:
					shakeCam = false;
            ...
            ease: FlxEase.expoOut
				...
				case 1416:
					resetCharacters();
					switchCharacter('dad'); //1
				case 1536:
					resetCharacters();
					switchCharacter('matt'); //1
				case 1600:
					resetCharacters();
					switchCharacter('tricky'); //1
				case 1664:
					resetCharacters();
					switchCharacter('bob'); //1
				case 1728:
					resetCharacters();
					switchCharacter('zardy'); //1
				case 1792:
					resetCharacters();
					switchCharacter('tricky'); //2
				case 1856:
					resetCharacters();
					switchCharacter('bob'); //2
				case 1984:
					resetCharacters();
					switchCharacter('whitty'); //1
				case 2112:
					resetCharacters();
					switchCharacter('zardy'); //2
				case 2240:
					resetCharacters();
					switchCharacter('dad'); //2
				case 2368:
					resetCharacters();
					switchCharacter('matt'); //2
				case 2496:
					resetCharacters();
					switchCharacter('whitty'); //2
				case 2624:
					resetCharacters();
					switchCharacter('dad');
					switchCharacter('matt');
            ...
            ease: FlxEase.expoOut
				...
				case 3136:
					if (Main.god)
					{
						resetCharacters();
						switchCharacter('dad');
						FlxTween.tween(FlxG.camera, {zoom: 0.7}, 3.2, {
						ease: FlxEase.expoOut
						});
					}
					else
					{
						resetCharacters();
						switchCharacter('bob');
					var eyebob:FlxSprite = new FlxSprite(-198, -118).loadGraphic(Paths.image('bobscreen', 'shared'));
					var bobsound:FlxSound = new FlxSound().loadEmbedded(Paths.sound('bobscreen'));
					...
					bobsound.volume = 1;
					bobb.visible = false;
					var runbob = new FlxSprite(450, 460);
					runbob.frames = Paths.getSparrowAtlas('hellbob_assets', 'shared');
					runbob.animation.addByPrefix('idle', "bobismad", 24);
					runbob.flipX = true;
					add(runbob);
					runbob.animation.play('idle');
					add(eyebob);
					new FlxTimer().start(0.5 , function(tmr:FlxTimer)
					{
						bobsound.volume = 0;
	...
	override function beatHit()
	{
		...
		ghostIcon.setGraphicSize(Std.int(ghostIcon.width + 30));
		...
		switch (curStage)
		{
			...
			case 'boxing':	
				if (Main.god)
				{
					if (bobDad == true)
					{
						if (FlxG.random.bool(5) && isbobmad)
						{
					...
					shagStartpos = new FlxPoint(dad2.x, dad2.y);
					mattStartpos = new FlxPoint(dad.x, dad.y);
					trickStartpos = new FlxPoint(tric.x, tric.y);
					bobStartpos = new FlxPoint(bobb.x, bobb.y);
					if (zardDad == true)
					{
						camHUD.alpha = 0.5;
					}
