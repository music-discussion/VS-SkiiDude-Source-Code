...
//	public static var snowSFX:Int = 0; //snow effect state trigger shit thing idk
	//snowSFX = _song.snowSFX;
	...
	//public static var snowSFX:Int = 0;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'skii':
			{
						defaultCamZoom = 0.9;
						curStage = 'skii';
						var bg:FlxSprite = new FlxSprite(310, 290).loadGraphic(Paths.image('newSkiiBG'));
						bg.antialiasing = true;
						...
						bgChar.animation.addByPrefix('bounce', 'skiiBGChar bounce bump', 24);
						bgChar.animation.play('bounce');
						bgChar.setGraphicSize(Std.int(bgChar.width * 1.1));
						bgChar.updateHitbox();
						bgChar.antialiasing = true;
						bgChar.scrollFactor.set(0.9, 0.9);
						bgChar.scale.set(4, 4);
						bgChar.y = 20;
						bgChar.x = -300;
				//		bgChar.active = true;
						add(bgChar);
...
*/
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.save.data.snowSFX == true && snowHit == true)
			{
				snowEffect.alpha = health + 0.5;
			
				snowEffect.setGraphicSize(Std.int(snowEffect.width * theFunneNumber));

					if (!snowTime.finished)
						snowHit == false;
						snowEffect.visible = false;
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if ((daNote.mustPress && daNote.tooLate && !PlayStateChangeables.useDownscroll || daNote.mustPress && daNote.tooLate && PlayStateChangeables.useDownscroll) && daNote.mustPress)
					{
						...
						switch (daNote.noteType) //easy
						{
					
							case 0: //normal, if you wanna fix it, try adding that in replay stuff if you want them to work properly, depends 1.5.4 lol
							{
							...
							case 1: //ice note go brrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr 
								daNote.kill();
								notes.remove(daNote, true);
								daNote.destroy();
						}
